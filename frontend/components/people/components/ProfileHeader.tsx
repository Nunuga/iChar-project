...
export const ProfileHeader = ({
  ...
  const createVCardURL = (prof: PersonDescriptionI) => {
    const vCardData = [
      "BEGIN:VCARD",
      "VERSION:3.0",
      `FN:${prof.name}`,
  ...
  return (
    <div className="bg-gray-800 bg-opacity-60 backdrop-blur-sm rounded-2xl shadow-2xl overflow-hidden mb-12">
      <div className="relative h-40 bg-gradient-to-r from-indigo-600 to-purple-600">
        <div className="absolute top-4 right-4 flex space-x-3">
          ...
          {professional.contact.github && (
            <a
              href={`https://${professional.contact.github}`}
              target="_blank"
              rel="noopener noreferrer"
              className="bg-white bg-opacity-20 p-2 rounded-full hover:bg-opacity-40 transition-all"
            >
          ...
          {professional.contact.twitter && (
            <a
              href={`https://${professional.contact.twitter}`}
              target="_blank"
              rel="noopener noreferrer"
              className="bg-white bg-opacity-20 p-2 rounded-full hover:bg-opacity-40 transition-all"
            >
        ...
        <div className="flex flex-col md:flex-row">
          <div className="relative -top-16 md:-top-20 flex-shrink-0">
            <img
              ...
              className="h-32 w-32 md:h-40 md:w-40 rounded-xl border-4 border-gray-800 shadow-lg object-cover"
            />
          </div>
          ...
          <div className="md:ml-6 md:pt-6 -mt-10 md:mt-0">
            <div className="flex flex-col md:flex-row md:items-center justify-between">
              ...
              <div className="mt-4 md:mt-0 space-y-2">
                ...
                {professional.contact.telegram && (
                  <div className="hidden md:flex items-center">
                    <RiTelegram2Fill className="h-5 w-5 text-indigo-400 mr-2" />
                    <a
                      href={`https://t.me/${professional.contact.telegram}`}
                      className="text-indigo-300 hover:text-white transition-colors underline decoration-dotted"
                    >
                      {"@" + professional.contact.telegram}
                    </a>
              ...
              <div className="md:hidden mt-4">
                ...
                <a
                  href={createVCardURL(professional)}
                  download={`${professional.name.replace(" ", "_")}.vcf`}
